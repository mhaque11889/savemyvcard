...
@section('main-section')
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">Processed Business Cards</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
          ...
          <div class="card">
            <div class="card-body">
              ...
              <table class="table" id="memberTable">
                <thead>
                  <tr><th>S.No</th><th>Image</th><th>Processed Data</th><th>Received at</th></tr>
                </thead>
                <tbody>
                      ...
                      <tr>
                          ...
                          <td>
                              <a href="https://smvcbusinesscards.s3.us-east-1.amazonaws.com/businesscards/{{ $card->media_id }}.jpeg" target="_blank">
                                  <img src="https://smvcbusinesscards.s3.us-east-1.amazonaws.com/businesscards/{{ $card->media_id }}.jpeg" style="max-height:100px;max-width:150px"/>
                              </a>
                          </td>
                          <td>
                              @php
                              ...
                              @if($data)
                                  ...
                                  <br/>
                                  <form action="{{ route('editScannedBusinessCards') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="media_id" value="{{ $card->media_id }}"/>
                                    <button class="btn btn-sm btn-outline-dark btnEditnResend">Edit Details</button>
                                  </form>
                              @else
                          ...
                          <td>
                            {{ \Carbon\Carbon::parse($card->created_at)->format('d-M-Y h:i A') }}<br/><br/>
                            {{-- Processed in <br/>{{ \Carbon\Carbon::parse($card->created_at)->diffInSeconds($card->updated_at) }} seconds --}}
                          </td>
                          {{-- <td>
  ...
  <script>
    ...
    $(".btnEditnResend").hover(
        function() {
            $(this).removeClass("btn-outline-dark").addClass("btn-danger").text("Coming Soon..");
        }, 
        function() {
            $(this).removeClass("btn-danger").addClass("btn-outline-dark").text("Edit Details");
        }
    );
    </script>
@endsection